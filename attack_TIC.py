...
def attack(args, model, filepath):    
    ...
    for i in range(steps):  
        ...
        x_size = (im_in.shape[2], im_in.shape[3])
        y = model.g_a(im_in, x_size)
        
        # noise or round
        # ste_round
        # y_hat = torch.round(y) - y.detach() + y
        
        x_hat = torch.clamp(model.g_s(y, x_size), min=0., max=1.)
        ...
        if i % 100 == 0:
            ...
            img.save("fake_in.png") 

            with torch.no_grad():
                y_round = torch.round(y)
                x_final = torch.clamp(model.g_s(y, x_size), min=0., max=1.)
...
def main(argv):
    ...
    snapshot = torch.load('best.pt')['model']
    net = TIC(config)
